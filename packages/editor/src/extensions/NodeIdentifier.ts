...
export interface NodeIdentifierOptions {
	/** Attribute persisted in Tiptap JSON and rendered into the node's HTML. */
	attributeName: string
	/** Node types that form addressable notebook/render blocks. */
	types: string[]
	/** Optional transaction filter, useful when a collaboration provider owns a change. */
	filterTransaction: ((transaction: Transaction) => boolean) | null
	/** Generates a fresh identifier for a missing or duplicated node ID. */
	generateID: (context: NodeIdentifierContext) => string
	/** Repairs existing documents and keeps identifiers current. */
...
/**
 * Open-source replacement for the former Tiptap Pro UniqueID dependency.
 *
 * IDs are the stable join key between notebook nodes, per-block rendering
 * configuration, recordings, and generated Hyperframes scenes. The plugin
 * repairs legacy content, regenerates duplicate IDs introduced by paste or
 * split operations, and leaves valid existing IDs untouched.
 */
