...
describe('NodeIdentifier', () => {
	it('repairs missing IDs and preserves valid existing IDs', async () => {
		...
		expect(identifiers[1]).not.toHaveLength(0)
		expect(editor.getHTML()).toContain('id="keep-me"')
		expect(editor.getHTML()).toContain(`id="${identifiers[1]}"`)
	})

	it('regenerates duplicate IDs introduced by pasted content', async () => {
		const editor = createEditor({
			type: 'doc',
			content: [
				{
	...
	it('gives a split node a new ID while retaining the original node ID', async () => {
		const editor = createEditor({
			type: 'doc',
			content: [
				{
