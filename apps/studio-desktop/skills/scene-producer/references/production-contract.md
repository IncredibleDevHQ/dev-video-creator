...
- `production/audio/narration.mp3` — the scene's voice, copied unchanged from
  `packet/audio/narration.mp3` (when the clock has one).
...
- `kind` is `generated-voice` (the product spoke the approved narration),
  `take` (the creator's recording) or `silent` (silent by the creator's
  choice; the moments keep the plan's estimates).
...
The sound plays from the start:

```html
<audio id="voice" src="audio/narration.mp3" data-start="0" data-duration="<clock duration>" data-track-index="20"></audio>
```

A scene the creator presents (`kind: take` with a `video`) plays the take's
picture in a presenter layer (`<video src="<clock video>">`, muted — the
sound is the clock's audio). A generated or silent scene has **no** presenter
layer and reserves no empty camera box.
...
- `controls` — optional: values the composition reads that the creator may
  change in the Studio. Each is a `hold` (seconds a moment holds its end
  state) or an `offset` (seconds a moment's action starts later), for one
  moment, with a default inside `min`–`max`. The code must read it by its id:
  `window.__controls?.["<id>"] ?? <default>`.
...
- a moment's interval is not the clock's, the composition is shorter than the
  ...
  packet's;
- the sound is missing from `production/`, no `<audio>` plays it, or the
...
- a control is not read by the code, or its default is outside its range;
