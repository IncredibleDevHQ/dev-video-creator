// Earlier-review P5 regression: a take's saved duration is fixed when the
// recording stops. Time spent reviewing the take (and the upload itself)
// never inflates it — a 3s take reviewed for a while still archives as 3s.
// Pattern per take-workflow-check.mjs (smoke app + __eval; __timing.stageReview
// stages the recorder's own review step with a fixed 3s stop-time duration).
import { spawn } from 'node:child_process'
import { mkdtemp, rm } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { join } from 'node:path'
import { fileURLToPath } from 'node:url'
import { createRequire } from 'node:module'

const require = createRequire(import.meta.url)
const appDir = fileURLToPath(new URL('..', import.meta.url))
const electronBinary = require('electron')
const root = await mkdtemp(join(tmpdir(), 'studio-take-duration-'))
const PROJECT_ID = `take-duration-${Date.now().toString(36)}`
const SCENE_ID = 'blk-timed'

const app = spawn(electronBinary, ['.', '--smoke', '--keep-running'], {
  cwd: appDir,
  env: {
    ...process.env,
    STUDIO_ALLOW_MULTI_INSTANCE: '1',
    STUDIO_DATA_DIR: join(root, 'data'),
    STUDIO_OUTPUTS_DIR: join(root, 'outputs'),
    STUDIO_ENABLE_TEST_HOOKS: '1',
  },
  stdio: ['ignore', 'pipe', 'inherit'],
})
const origin = await new Promise((resolve, reject) => {
  let buffer = ''
  const timeout = setTimeout(() => reject(new Error('app start timed out')), 90_000)
  app.stdout.on('data', chunk => {
    buffer += chunk
    const match = /STUDIO_ORIGIN (http:\/\/\S+)/.exec(buffer)
    if (match && buffer.includes('SMOKE PASS')) { clearTimeout(timeout); resolve(match[1]) }
  })
  app.once('exit', code => reject(new Error(`app exited (${code})`)))
})

let failures = 0
const check = (label, ok, detail = '') => {
  console.log(`${ok ? 'PASS' : 'FAIL'}  ${label}${detail ? `  ${detail}` : ''}`)
  if (!ok) failures += 1
}
const evalInWindow = async js => {
  const response = await fetch(`${origin}/__eval`, {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ js }),
  }).then(r => r.json())
  if (!response.ok) throw new Error(response.error || 'eval failed')
  return response.result
}
const sleep = ms => new Promise(r => setTimeout(r, ms))
const waitFor = async (js, timeoutMs = 30_000) => {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    const value = await evalInWindow(js).catch(() => null)
    if (value) return value
    await sleep(400)
  }
  throw new Error(`waitFor timed out: ${js.slice(0, 90)}`)
}

try {
  const project = {
    version: 1, derivedFrom: { notebook: 'fixture-base', kind: 'video' }, id: PROJECT_ID, title: 'Take duration fixture',
    notebook: { type: 'doc', content: [
      { type: 'heading', attrs: { id: 'blk-h1', level: 1 }, content: [{ type: 'text', text: 'Duration' }] },
      { type: 'scene', attrs: { id: SCENE_ID, title: 'Timed scene', script: 'Three seconds of delivery.' } },
    ] },
    fps: 30, width: 1920, height: 1080, blocks: {}, presenterTracks: {}, recordedBlocks: {}, brand: {}, theme: {},
  }
  await fetch(`${origin}/api/projects/${PROJECT_ID}`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify(project) })
  await evalInWindow(`localStorage.setItem('incredible-studio-v2-active-project', '${PROJECT_ID}'); location.assign('/studio')`)
  await waitFor(`!!document.getElementById('${SCENE_ID}')`)

  await evalInWindow(`(() => {
    const chip = [...document.querySelectorAll('#scene-rail .scene-card')].find(b => b.textContent.includes('Timed scene'))
    if (!chip) throw new Error('no scene chip')
    chip.click()
    document.getElementById('record-this-block').click()
  })()`)
  await waitFor(`document.getElementById('camera-dialog')?.open === true`)

  // A 3s take stops into review; the creator then reviews it for a while
  // before keeping it.
  await evalInWindow(`window.__timing.stageReview()`)
  await sleep(2_500)
  await evalInWindow(`document.getElementById('keep-take').click()`)
  let archived = null
  for (let i = 0; i < 30; i += 1) {
    archived = await fetch(`${origin}/api/takes?projectId=${PROJECT_ID}`).then(r => r.json()).catch(() => null)
    if (archived?.takes?.length === 1) break
    await sleep(400)
  }
  check(
    'the archived duration is the recording length, not recording plus review',
    archived?.takes?.[0]?.durationMs === 3000,
    `durationMs=${archived?.takes?.[0]?.durationMs}`,
  )

  let doc = null
  for (let i = 0; i < 20; i += 1) {
    doc = await fetch(`${origin}/api/projects/${PROJECT_ID}`).then(r => r.json()).then(b => b.project).catch(() => null)
    if (doc?.recordedBlocks?.[SCENE_ID]) break
    await sleep(400)
  }
  check(
    'the active take on the notebook keeps the stop-time duration',
    doc?.recordedBlocks?.[SCENE_ID]?.durationMs === 3000,
    `durationMs=${doc?.recordedBlocks?.[SCENE_ID]?.durationMs}`,
  )

  await fetch(`${origin}/api/projects/${PROJECT_ID}`, { method: 'DELETE' })
  check('cleanup', true, 'fixture notebook deleted')
} catch (error) {
  check(`run: ${error.message}`, false)
} finally {
  app.kill('SIGTERM')
  await sleep(600)
  await rm(root, { recursive: true, force: true })
}
console.log(failures ? `TAKE DURATION CHECK FAIL (${failures})` : 'TAKE DURATION CHECK PASS')
process.exitCode = failures ? 1 : 0
