...
try {
  ...
  const r5 = await planned(s1, 5)
  await previewIt(5)
  await evaluate(`() => { location.reload(); return true }`).catch(() => {})
  await sleep(3000)
  await waitFor(`() => document.querySelector('#scene-workspace .sw-scene.is-selected')?.dataset.scene === ${JSON.stringify(s1)} ? true : null`, 60)
  await release(s1, 5)
  const url6 = await until(() => previewUrl(videoId, s1, r5.id), 90)
  ...
  check(reopened?.mode === 'preview' && reopened.scene === s1, `reopened while it was made, the preview is still handed over when ready (${JSON.stringify({ mode: reopened?.mode, scene: reopened?.scene })})`)
